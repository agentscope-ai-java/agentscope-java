...
To bring **DeepSeek Harness** into the same fleet, load the Cordis plugin at [`agentscope-service/aistio/sdk/dsh`](aistio/sdk/dsh) (`@agentscope/dsh-aistio`). It self-registers with aistiod, serves `/agentscope/*`, receives AgentTask events, and uses the same Issue/Comment/Artifact contract as other runtimes. See that directory's [README](aistio/sdk/dsh/README.md).
